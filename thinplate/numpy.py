...
class TPS:       
    ...
    @staticmethod
    def u(r):
        return np.square(r) * np.log(r + 1e-6)
    ...
    def z(x, c, theta):
        ...
        #print("Atleast : %gms" % (t*1000/cv2.getTickFrequency()))
        dist = TPS.d(x, c)
        #t = cv2.getTickCount() - st
        #print("2D dist : %gms" % (t*1000/cv2.getTickFrequency()))

        U = TPS.u(dist)
        #t = cv2.getTickCount() - st
        #print("U : %gms" % (t*1000/cv2.getTickFrequency()))
        ...
        #t = cv2.getTickCount() - st
        #print("Reduced : %gms" % (t*1000/cv2.getTickFrequency()))
        b = np.dot(U, w)
        #t = cv2.getTickCount() - st
        #print("Dot : %gms" % (t*1000/cv2.getTickFrequency()))
...
def tps_grid(theta, c_dst, dshape):    
    #t = cv2.getTickCount()
    h, w = dshape
    gh, gw = min(64, h//8), min(64, w//8)
    ori_grid = uniform_grid(dshape)
    ugrid = uniform_grid([gh, gw])

    #t = cv2.getTickCount() - t
    #print("Uniform grid: %gms" % (t*1000/cv2.getTickFrequency()))

    #reduced = c_dst.shape[0] + 2 == theta.shape[0]

    dx = TPS.z(ugrid.reshape((-1, 2)), c_dst, theta[:, 0]).reshape([gh, gw])
    dy = TPS.z(ugrid.reshape((-1, 2)), c_dst, theta[:, 1]).reshape([gh, gw])

    tmp = np.zeros((gh, gw, 3), dtype=np.float)
    tmp[:, :, :2] = np.stack((dx, dy), -1)
    dgrid = cv2.resize(tmp, (w, h), interpolation=cv2.INTER_CUBIC)[:, :, :2]
